...
class ProfileVC: UIViewController, UITextFieldDelegate {
    ...
    func setupNavigationBar(){
        ...
        navigationController?.navigationBar.topItem?.title = "Изменить мой данные"
    ...
    let lastnameInput: UITextField = {
        let input =  BFTextField(frame: .zero)
        input.placeholder = "Ваш фамиля:"
        input.title = "Ваш фамиля:"
        input.text = "Maine"
        input.translatesAutoresizingMaskIntoConstraints = false
        return input
    }()
    let emailInput: UITextField = {
        ...
        input.title = "E-Mail:"
        input.text = "rubymaine@gmail.com"
        input.translatesAutoresizingMaskIntoConstraints = false
        return input
    }()
    let phoneInput: UITextField = {
        let input =  BFTextField(frame: .zero)
        input.placeholder = "Ваш телефон номер:"
        input.title = "Ваш телефон номер:"
        input.text = "+998 00 123-45-67"
        input.alpha = 0.8
