...
extension HomeVC: UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout{
    ...
    func collectionView(_ collectionView: UICollectionView, viewForSupplementaryElementOfKind kind: String, at indexPath: IndexPath) -> UICollectionReusableView {
        ...
        if indexPath.section == 0 {
            view.headingLabel.text = "🎁 Дискоунтная скидка на всё меню"
            view.headingButton.isHidden = false
            view.headingButton.alpha = 1
        }
        else if indexPath.section == 1 {
            view.headingLabel.text = "Бесплатная доставка"
            view.headingButton.isHidden = false
            view.headingButton.alpha = 1
        }
        else if indexPath.section == 2 {
            view.headingLabel.text = "Самые популярные"
            view.headingButton.isHidden = false
            view.headingButton.alpha = 1
        }
        else if indexPath.section == 3 {
            view.headingLabel.text = "Популярные"
            view.headingButton.isHidden = false
            view.headingButton.alpha = 1
        }
        else if indexPath.section == 4 {
            view.headingLabel.text = "Все из ресторанны и магазина"
