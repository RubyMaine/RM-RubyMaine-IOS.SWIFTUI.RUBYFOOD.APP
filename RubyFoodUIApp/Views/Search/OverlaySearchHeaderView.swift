...
class OverlaySearchHeaderView: UIView {
    ...
    let searchTextField: UITextField = {
        var tf = UITextField()
        tf.attributedPlaceholder = NSAttributedString(string: "Поискать и перекусать ...", attributes: [.foregroundColor: UIColor.systemGray3,.font:UIFont(name: font.medium.rawValue, size: 12)!])
        tf.borderStyle = .none
        tf.textColor = .black
