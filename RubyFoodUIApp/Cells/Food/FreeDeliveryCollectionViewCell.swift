...
class FreeDeliveryCollectionViewCell: UICollectionViewCell {
    ...
    let foodDescriptionView: FoodDescriptionDetailsView = {
        ...
        v.actualPriceLabel.attributedText = NSMutableAttributedString(attributedString: NSAttributedString(string: "50.00 Сум", attributes: [NSAttributedString.Key.strikethroughStyle: NSUnderlineStyle.single.rawValue,NSAttributedString.Key.strikethroughColor: color.grey]))
    ...
    func manageData(){
        ...
        if let discountPrice = item.discountPrice {
            foodDescriptionView.discountPriceLabel.text = "\(discountPrice.twoDecimalPlaces()) Сум"
        }
        if let discountPercent = item.discountPercent {
            ...
            foodDescriptionView.discountedContainer.isHidden = true
            foodDescriptionView.actualPriceLabel.attributedText =  NSAttributedString(string: "\(item.price.twoDecimalPlaces()) Сум", attributes: [.foregroundColor: color.black])
            imageDescriptionView.discountContainer.isHidden = true
        }
    ...
    func setupAttributedText (_ price: String) -> NSAttributedString {
        let text = NSMutableAttributedString(attributedString: NSAttributedString(string: "\(price) Сум", attributes: [NSAttributedString.Key.strikethroughStyle: NSUnderlineStyle.single.rawValue,NSAttributedString.Key.strikethroughColor: color.grey,.foregroundColor: color.grey]))
        return text
    }
}
