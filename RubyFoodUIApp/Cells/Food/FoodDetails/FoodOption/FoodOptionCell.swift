...
class FoodOptionCell: UITableViewCell {
    ...
    let titleLabel: UILabel = {
        let lb = UILabel()
        lb.text = "10% скидка на выбранного сеты!"
        lb.scaleFont()
        lb.numberOfLines = 1
    ...
    func manageData(){
        ...
        if let discountPrice = item.discountPrice {
            ...
            discountPriceLabel.text = "\(discountPrice.twoDecimalPlaces()) Сум"
            discountedContainer.isHidden = false
        }else{
            priceLabel.attributedText = NSAttributedString(string: "\(item.price.twoDecimalPlaces()) Сум", attributes: [.foregroundColor: color.black])
            discountedContainer.isHidden = true
        }
